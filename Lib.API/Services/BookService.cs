...
public class BookService: IBookService
{
    ...
    public async Task<IEnumerable<Book>> SearchTermAsync(string searchTerm)
    {
        using var connection = await _connectionFactory.CreateConnectionAsync();
    ...
    {
        var bookToUpdate = await GetByIsbnAsync(book.Isbn);
        if (bookToUpdate is null)
        {
            return false;
        }
        using var connection = await _connectionFactory.CreateConnectionAsync();
        var results = await connection.ExecuteAsync("UPDATE Books SET Title = @Title, Author = @Author , " +
                                          "ShortDescription = @ShortDescription, PageCount = @PageCount, " +
